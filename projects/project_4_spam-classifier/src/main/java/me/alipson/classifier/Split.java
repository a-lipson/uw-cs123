...
public class Split {
    ...
    // containing two elements separated by Classifiable.SPLITTER, return the first
    // part
    public String getFeature() {
        return feature.split(Classifiable.SPLITTER)[0];
    }

    // Evaluates the provided Classifiable object on this split, returning true if
    // it falls
    // below (<) this split, false if it falls above (>=) this split
    public boolean evaluate(Classifiable value) {
        return value.get(this.feature) < this.threshold;
    }

    // Returns a String representation of this split, with the feature on the first
    // line preceeded
    ...
    // Returns the value that falls exactly in the middle of the two provided
    // doubles
